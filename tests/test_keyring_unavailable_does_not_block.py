...
def test_load_config_does_not_raise_when_keyring_returns_empty(
    ...
    assert cfg.agent_password == "", (
        "Token doit être vide pour permettre le démarrage en mode dégradé"
    )
    assert any(
        "Trousseau" in rec.message or "trousseau" in rec.message
