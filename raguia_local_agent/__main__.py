...
def main() -> None:
    ...
    try:
        ...
        if not cfg.agent_token and not args.test:
            # Le trousseau OS peut être temporairement indisponible immédiatement
            # après un redémarrage (Windows Credential Locker, GNOME Keyring,
            # etc.). On retente plusieurs fois avec un court délai avant de
